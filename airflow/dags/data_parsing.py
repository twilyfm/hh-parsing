...
def parsing(vacancy_name: str) -> pd.DataFrame:
    ...
    url = f'''https://hh.ru/search/vacancy?ored_clusters=
              true&search_period=1&search_field=name&search_field=
              description&enable_snippets=false&text={vacancy_name}'''
    ...
    for vacancy in tqdm(vacancies):
        ...
        # vacancy name
        name = driver.find_elements(By.CSS_SELECTOR, '''[data-qa="vacancy-title"]''')
        ...
        # city
        city = driver.find_elements(By.CSS_SELECTOR, '''[data-qa=vacancy-view-raw-address]''')
        city = [x.text for x in city]
        if not city:
            city = driver.find_elements(By.CSS_SELECTOR, '''[data-qa=vacancy-view-location]''')
            city = [x.text for x in city]
